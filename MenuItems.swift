...
class MenuItems {
    ...
    init() {
        ...
        // TODO rethink this
        for item in testItems { if item.type == MenuItems.foodTypes.APP { apps.append(item) } }
        for item in testItems { if item.type == MenuItems.foodTypes.SALAD { salads.append(item) } }
        for item in testItems { if item.type == MenuItems.foodTypes.PANINI { panini.append(item) } }
        for item in testItems { if item.type == MenuItems.foodTypes.BASKET { baskets.append(item) } }
        for item in testItems { if item.type == MenuItems.foodTypes.COLD_SANDWICH { coldSandwiches.append(item) } }
        for item in testItems { if item.type == MenuItems.foodTypes.HOT_SANDWICH { hotSandwiches.append(item) } }
        for item in testItems { if item.type == MenuItems.foodTypes.WRAP { wraps.append(item) } }
        for item in testItems { if item.type == MenuItems.foodTypes.PIZZA { pizzas.append(item) } }
        for item in testItems { if item.type == MenuItems.foodTypes.CALZONE { calzones.append(item) } }
        for item in testItems { if item.type == MenuItems.foodTypes.SIDE { sides.append(item) } }
        for item in testItems { if item.type == MenuItems.foodTypes.BEVERAGE { beverages.append(item) } }
